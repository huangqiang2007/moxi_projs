...
//#define KEY_TEMP_PLUS_PIN      GPIO_Pin_5

//uint8_t System_ON_OFF = 0 ; // 0: system off; 1: system on.
//uint8_t Timing_flag = 0 ; // 0: 10 minutes; 1: 15 minutes; 2: 20 minutes.
//uint8_t Power_rank_flag = 0; //0: rank1 ; 1: rank2 ; 2: rank3

// from time.c
//extern uint16_t TimerCounter;
//extern uint16_t Channel0Pulse,Channel625Pulse,Channel875Pulse;
//extern uint16_t Channel25Pulse,Channel50Pulse,Channel75Pulse,Channel100Pulse;
...
//extern uint8_t Computer_Results_flage;
...
//	R_value_Old = 254;  //  last resistance value of product.(default)
	...
	{
//		sprintf(Tx_Buffer,"\r\n [INFO]  Close_CMOS()LOAD_DETECT_exception_flag(key.c-167 line)  \r\n");
//		UART_Send( Tx_Buffer, (uint32_t) strlen(Tx_Buffer));
		...
		for(i = 0;i<7; i++)//取前7次的转换结果
	{
		...
		if (connect_loader_start_counter >= No_connect_loader_start_counter) No_connect_loader_start_flag = 0;
		else No_connect_loader_start_flag = 1;
...
{
	if(LOAD_DETECT_exception_flag ==1||No_connect_loader_close_flag ==1||TimeOut_flag == 1||Res_Value_Exception_flag ==1||Output_Voltage_exception_flag ==1||VIN_DETECT_exception_flag ==1)
			{	
...
//									UART_Send( Tx_Buffer, (uint32_t) strlen(Tx_Buffer));
		...
		if( KEY_Down(GPIOB,KEY_TIME_SELECT_PIN) == 0 && System_ON_OFF == 1 &&No_connect_loader_flag ==0)//判定定时选择按键是否按下
			{
