...
extern void EscButton(uint16_t xstart,uint16_tystart,uint8_t xend,uint16_t yend); // 显示按钮松开状态
extern void Draw_TextBox(uint16_t xstart, uint16_t ystart, uint16_t xend, uint16_t yend);	// 显示一个输入框
extern void Draw_Window(uint16_t xstart,uint16_t ystart,uint8_t xend,uint16 yend,uint8_t* caption);	// 显示一个WINDOWS窗口
